...
def plot(values, best):
    plt.plot(time[:len(values)], values,                    'k', label='Исходный', linewidth=0.5)
    plt.plot(time[:len(values)], best,                      'b', label='Остатки',  linewidth=0.5)
    plt.plot(time[:len(values)], np.subtract(values, best), 'r', label='Модель',   linewidth=1)
...
def first_method(values):
    ...
    for n in range(2, 11, 2):
        ...
        harmonic.append(np.sin(n * np.pi * t / T))
        harmonic.append(np.cos(n * np.pi * t / T))
